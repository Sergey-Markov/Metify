...
import {
  ...
  Platform, Pressable, LayoutAnimation,
} from 'react-native';
...
import { useGoalsSummary, useGoalActions } from '../../src/hooks/goalsHabits';
import { daysUntilGoal, computeGoalProgress, makeGoalId } from '../../src/utils/goalsHabits';
import type { Goal, GoalCategory, GoalPriority, Milestone } from '../../src/types/goalsHabits';
...
const PRI_LABELS: Record<GoalPriority, string> = { high: '●', medium: '◐', low: '○' };
const PRI_COLORS: Record<GoalPriority, string> = { high: '#e05a5a', medium: '#c8a96e', low: '#8a8a9a' };
...
export default function GoalsScreen() {
  ...
  const { addGoal, updateGoal, completeGoal, deleteGoal, addMilestone, toggleMilestone } =
    useGoalActions();
  ...
  const liveSelectedGoal = useMemo(() =>
    selectedGoal ? activeGoals.find(g => g.id === selectedGoal.id) ?? selectedGoal : null,
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      {activeGoals.length > 0 && (
        <View style={s.overallRow}>
          ...
          <View style={s.overallTrack}>
            <Animated.View style={[s.overallFill, { width: `${avgProgress}%` }]} />
          </View>
        </View>
      )}
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.filterRow} contentContainerStyle={s.filterContent}>
        {CATEGORIES.map(c => (
          ...
          >
            {c.value !== 'all' && (
              <View style={[s.filterDot, { backgroundColor: CAT_COLORS[c.value] }]} />
            )}
            <Text style={[s.filterText, filterCat === c.value && s.filterTextActive]}>
              {c.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* Active goals */}
        {filtered.length === 0 && (
          <EmptyGoals onAdd={() => setShowAdd(true)} filtered={filterCat !== 'all'} />
        )}
        ...
        {completedGoals.length > 0 && (
          <View style={s.completedSection}>
            <TouchableOpacity style={s.completedToggle} onPress={toggleDone}>
              <Text style={s.completedToggleText}>
                Виконані цілі ({completedGoals.length})
              </Text>
              <Text style={s.completedArrow}>{showDone ? '▲' : '▼'}</Text>
            </TouchableOpacity>
            {showDone && completedGoals.map(g => (
              <GoalCard key={g.id} goal={g} completed onDelete={() => deleteGoal(g.id)} />
            ))}
          </View>
        )}
      ...
      {liveSelectedGoal && (
        <GoalDetailSheet
          ...
          onClose={closeDetail}
          onComplete={() => { completeGoal(liveSelectedGoal.id); closeDetail(); }}
      ...
      {showAdd && (
        <AddGoalSheet
          ...
          onSave={(draft) => { addGoal(draft); setShowAdd(false); }}
        />
      )}
...
function GoalCard({ goal, onPress, onComplete, onDelete, completed }: {
  goal: Goal;
  onPress?: () => void;
  onComplete?: () => void;
  onDelete?: () => void;
  ...
  return (
    ...
    >
      ...
      <View style={s.goalCardTop}>
        <View style={[s.catPill, { backgroundColor: catColor + '18', borderColor: catColor + '40' }]}>
      ...
      <View style={s.progressTrack}>
        <View style={[s.progressFill, { width: `${goal.progress}%`, backgroundColor: completed ? colors.green : catColor }]} />
      </View>
...
function GoalDetailSheet({ goal, onClose, onComplete, onToggleMilestone, onAddMilestone }: {
  goal: Goal;
  onClose: () => void;
  onComplete: () => void;
  ...
  return (
    <Animated.View style={s.overlay} entering={FadeIn} exiting={FadeOut}>
      ...
      <Animated.View style={s.sheet} entering={SlideInDown} exiting={SlideOutDown}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={[s.detailBanner, { backgroundColor: catColor + '14' }]}>
            <View style={s.detailTopRow}>
              <View style={[s.catPill, { backgroundColor: catColor + '20', borderColor: catColor + '40' }]}>
                <View style={[s.catPillDot, { backgroundColor: catColor }]} />
                <Text style={[s.catPillText, { color: catColor }]}>
                  {CATEGORIES.find(c => c.value === goal.category)?.label}
                </Text>
              ...
              <Text style={[s.priIcon, { color: PRI_COLORS[goal.priority] }]}>
                {PRI_LABELS[goal.priority]} {goal.priority === 'high' ? 'Важливо' : goal.priority === 'medium' ? 'Середньо' : 'Низьке'}
              </Text>
            </View>
            ...
            <View style={s.detailStats}>
              <View style={s.detailStat}>
                <Text style={[s.detailStatVal, { color: catColor }]}>{goal.progress}%</Text>
                <Text style={s.detailStatLabel}>Прогрес</Text>
              </View>
              <View style={s.detailDivider} />
              <View style={s.detailStat}>
                <Text style={s.detailStatVal}>{doneMiles}/{goal.milestones.length}</Text>
                <Text style={s.detailStatLabel}>Кроків</Text>
              </View>
              <View style={s.detailDivider} />
              <View style={s.detailStat}>
                <Text style={[s.detailStatVal, daysLeft === 0 && { color: colors.red }]}>
                  {daysLeft === null ? '—' : daysLeft === 0 ? '!' : daysLeft}
                </Text>
                <Text style={s.detailStatLabel}>{daysLeft === 0 ? 'Прострочено' : 'Днів'}</Text>
              </View>
            </View>

            {/* Progress bar */}
            <View style={[s.progressTrack, { marginTop: 8, height: 4 }]}>
              <View style={[s.progressFill, { width: `${goal.progress}%`, backgroundColor: catColor }]} />
            </View>
          </View>
          ...
          <View style={s.milesSection}>
            ...
            {goal.milestones.map(m => (
              ...
              >
                ...
                <Text style={[s.msTitle, m.completed && s.msTitleDone]}>{m.title}</Text>
              </TouchableOpacity>
            ))}
            ...
            <View style={s.addMsRow}>
              <TextInput
                ...
                returnKeyType="done"
              />
              <TouchableOpacity style={s.addMsBtn} onPress={handleAddMs}>
          ...
          <View style={s.detailActions}>
            {goal.progress === 100 && (
              <TouchableOpacity style={s.completeBig} onPress={onComplete}>
...
function AddGoalSheet({ onClose, onSave }: {
  ...
  return (
    <Animated.View style={s.overlay} entering={FadeIn} exiting={FadeOut}>
      <Pressable style={StyleSheet.absoluteFill} onPress={onClose} />
      <Animated.View style={[s.sheet, { maxHeight: '90%' }]} entering={SlideInDown} exiting={SlideOutDown}>
        <View style={s.sheetHandle} />
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 14 }}>
            {CATEGORIES.filter(c => c.value !== 'all').map(c => (
              ...
              >
                <View style={[s.catPillDot, { backgroundColor: CAT_COLORS[c.value] }]} />
                <Text style={[s.catChipText, category === c.value && { color: CAT_COLORS[c.value] }]}>
                  {c.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={s.freqRow}>
            {(['high', 'medium', 'low'] as GoalPriority[]).map(p => (
              <TouchableOpacity
                key={p}
                style={[s.freqOpt, priority === p && { borderColor: PRI_COLORS[p], backgroundColor: PRI_COLORS[p] + '14' }]}
                onPress={() => setPriority(p)}
              >
                <Text style={[s.freqText, priority === p && { color: PRI_COLORS[p] }]}>
                  {PRI_LABELS[p]} {p === 'high' ? 'Важливо' : p === 'medium' ? 'Середній' : 'Низький'}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {milestones.map((m, i) => (
            <View key={i} style={s.msBuilderRow}>
              <TextInput
              ...
              />
              {milestones.length > 1 && (
                <TouchableOpacity style={s.msRemoveBtn} onPress={() => removeMsField(i)}>
                  <Text style={s.msRemoveBtnText}>✕</Text>
                </TouchableOpacity>
              )}
            </View>
          ))}
          <TouchableOpacity style={s.msAddField} onPress={addMsField}>
...
function EmptyGoals({ onAdd, filtered }: { onAdd: () => void; filtered: boolean }) {
  return (
    <View style={s.empty}>
...
const s = StyleSheet.create({
  ...
  headerSub:         { fontSize: 11, letterSpacing: 1.5, color: colors.muted, textTransform: 'uppercase', marginBottom: 2 },
  headerTitle:       { fontFamily: SERIF, fontSize: 32, color: colors.text },
  addBtn:            { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(200,169,110,0.12)', borderWidth: 0.5, borderColor: 'rgba(200,169,110,0.3)', alignItems: 'center', justifyContent: 'center' },
  addBtnText:        { fontSize: 22, color: colors.accent, lineHeight: 26 },

  overallRow:        { paddingHorizontal: 24, paddingBottom: 10 },
  overallLabel:      { fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: colors.muted, marginBottom: 4 },
  overallPct:        { fontFamily: SERIF, fontSize: 18, color: colors.accent, marginBottom: 6 },
  overallTrack:      { height: 2, backgroundColor: colors.subtle, borderRadius: 1, overflow: 'hidden' },
  overallFill:       { height: '100%', backgroundColor: colors.accent, borderRadius: 1 },

  filterRow:         { paddingBottom: 12 },
  filterContent:     { paddingHorizontal: 24, gap: 8 },
  filterChip:        { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, borderWidth: 0.5, borderColor: colors.subtle, backgroundColor: colors.bg2 },
  filterChipActive:  { borderColor: colors.accent, backgroundColor: 'rgba(200,169,110,0.08)' },
  ...
  // Goal card
  goalCard:          { marginHorizontal: 24, marginBottom: 12, backgroundColor: colors.bg2, borderRadius: 16, padding: 16, borderWidth: 0.5, borderColor: colors.subtle },
  goalCardDone:      { opacity: 0.5 },
  goalCardTop:       { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  catPill:           { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, borderWidth: 0.5 },
  ...
  goalTitle:         { fontFamily: SERIF, fontSize: 18, color: colors.text, marginBottom: 8, lineHeight: 24 },
  ...
  progressTrack:     { height: 3, backgroundColor: colors.subtle, borderRadius: 2, overflow: 'hidden', marginBottom: 6 },
  progressFill:      { height: '100%', backgroundColor: colors.accent, borderRadius: 2 },
  progressRow:       { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  progressPct:       { fontSize: 11, color: colors.muted },
  completeBtn:       { backgroundColor: 'rgba(78,203,141,0.12)', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 0.5, borderColor: 'rgba(78,203,141,0.3)' },
  completeBtnText:   { fontSize: 11, color: colors.green },

  // Completed section
  completedSection:  { paddingHorizontal: 24, marginBottom: 12 },
  completedToggle:   { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderTopWidth: 0.5, borderTopColor: colors.subtle },
  completedToggleText:{ fontSize: 12, color: colors.muted, letterSpacing: 1 },
  completedArrow:    { fontSize: 10, color: colors.muted },

  // Detail sheet
  overlay:           { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.75)', justifyContent: 'flex-end', zIndex: 100 },
  sheet:             { backgroundColor: colors.bg2, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 0, maxHeight: '85%' },
  sheetHandle:       { width: 40, height: 3, backgroundColor: colors.subtle, borderRadius: 2, alignSelf: 'center', marginBottom: 20 },
  sheetTitle:        { fontFamily: SERIF, fontSize: 24, color: colors.text, marginBottom: 20 },
  ...
  detailTitle:       { fontFamily: SERIF, fontSize: 22, color: colors.text, lineHeight: 28, marginBottom: 12 },
  detailStats:       { flexDirection: 'row', alignItems: 'center', gap: 0, marginBottom: 4 },
  detailStat:        { flex: 1, alignItems: 'center' },
  detailStatVal:     { fontFamily: SERIF, fontSize: 22, color: colors.text },
  detailStatLabel:   { fontSize: 9, letterSpacing: 1, textTransform: 'uppercase', color: colors.muted, marginTop: 2 },
  detailDivider:     { width: 0.5, height: 36, backgroundColor: colors.subtle },
  detailActions:     { gap: 10, marginTop: 20 },
  completeBig:       { backgroundColor: 'rgba(78,203,141,0.12)', borderRadius: 14, paddingVertical: 14, alignItems: 'center', borderWidth: 0.5, borderColor: 'rgba(78,203,141,0.3)' },
  completeBigText:   { fontSize: 14, color: colors.green, fontWeight: '500' },
  closeSheetBtn:     { backgroundColor: 'rgba(200,169,110,0.08)', borderRadius: 14, paddingVertical: 14, alignItems: 'center', borderWidth: 0.5, borderColor: 'rgba(200,169,110,0.15)' },
  closeSheetBtnText: { fontSize: 14, color: colors.accent },

  // Milestones
  milesSection:      { marginBottom: 4 },
  milesTitle:        { fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: colors.muted, marginBottom: 12, fontWeight: '500' },
  milestone:         { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10, borderBottomWidth: 0.5, borderBottomColor: colors.subtle },
  msCheck:           { width: 20, height: 20, borderRadius: 6, borderWidth: 0.5, borderColor: colors.subtle, alignItems: 'center', justifyContent: 'center' },
  ...
  addMsInput:        { flex: 1, backgroundColor: colors.bg, borderWidth: 0.5, borderColor: colors.subtle, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, color: colors.text, fontSize: 13 },
  addMsBtn:          { width: 40, height: 40, borderRadius: 10, backgroundColor: 'rgba(200,169,110,0.12)', borderWidth: 0.5, borderColor: 'rgba(200,169,110,0.3)', alignItems: 'center', justifyContent: 'center' },
  addMsBtnText:      { fontSize: 20, color: colors.accent, lineHeight: 22 },

  // Add sheet form
  input:             { backgroundColor: colors.bg, borderWidth: 0.5, borderColor: colors.subtle, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 13, color: colors.text, fontSize: 15, marginBottom: 14 },
  fieldLabel:        { fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: colors.muted, marginBottom: 8, fontWeight: '500' },
  catChip:           { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 0.5, borderColor: colors.subtle, marginRight: 8 },
  catChipText:       { fontSize: 12, color: colors.muted },
  freqRow:           { flexDirection: 'row', gap: 8, marginBottom: 16 },
  freqOpt:           { flex: 1, paddingVertical: 10, borderRadius: 10, borderWidth: 0.5, borderColor: colors.subtle, alignItems: 'center' },
  freqText:          { fontSize: 11, color: colors.muted },
  msBuilderRow:      { flexDirection: 'row', gap: 8, marginBottom: 8, alignItems: 'center' },
  msRemoveBtn:       { width: 36, height: 44, alignItems: 'center', justifyContent: 'center' },
  msRemoveBtnText:   { fontSize: 14, color: colors.muted },
  msAddField:        { paddingVertical: 10, alignItems: 'center', borderWidth: 0.5, borderColor: colors.subtle, borderRadius: 10, borderStyle: 'dashed' },
  msAddFieldText:    { fontSize: 13, color: colors.muted },
  sheetBtns:         { flexDirection: 'row', gap: 10 },
  btnCancel:         { flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 0.5, borderColor: colors.subtle, alignItems: 'center' },
  btnCancelText:     { fontSize: 14, color: colors.muted },
  btnSave:           { flex: 2, paddingVertical: 14, borderRadius: 12, backgroundColor: colors.accent, alignItems: 'center' },
  btnSaveText:       { fontSize: 14, fontWeight: '500', color: colors.bg },

  // Empty
  empty:             { alignItems: 'center', padding: 48 },
  emptyIcon:         { fontSize: 48, marginBottom: 16, opacity: 0.4 },
  emptyTitle:        { fontFamily: SERIF, fontSize: 24, color: colors.text, marginBottom: 8 },
  emptySub:          { fontSize: 13, color: colors.muted, textAlign: 'center', lineHeight: 20, marginBottom: 24 },
  emptyBtn:          { backgroundColor: 'rgba(200,169,110,0.12)', borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12, borderWidth: 0.5, borderColor: 'rgba(200,169,110,0.3)' },
